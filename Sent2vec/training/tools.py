...
def train_regressor(options, embed_map, wordvecs, worddict):
    ...
    d = defaultdict(lambda : 0)
    #print(embed_map[u'ゴマ'])
    for w in embed_map.wv.vocab.keys():
        #ここもおｋ
        #print "w:"+w
        d[w] = 1
    shared = OrderedDict()
    ...
    for w in shared.keys():
        ...
        sg[shared[w]] = wordvecs[w.encode('utf-8')] #wordvecs=OrderedDict
...
def apply_regressor(clf, embed_map, use_norm=False):
    ...
    for i, w in enumerate(embed_map.wv.vocab.keys()):
        if '_' not in w:
            wordvecs[w] = clf.predict(embed_map[w].reshape(1,-1)).astype('float32') #エラー
            if use_norm:
